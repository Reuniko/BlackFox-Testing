...
class TestCache extends Test {
	...
	public function __construct() {
		parent::__construct();
		$this->CACHE = \BlackFox\Cache::I();
		$this->CACHE->Clear();
	}

	public function TestCacheIsNotDefaultEngine() {
		if (get_class($this->CACHE) === 'BlackFox\Cache') {
			throw new Exception("You are using default cache engine (BlackFox\Cache), you should switch to other engine");
		}
	...
	public function TestPutAndGetObject() {
		$random_user_id = \BlackFox\Users::I()->Read([], ['ID'], ['{RANDOM}' => 'ASC'])['ID'];
		$value = new \BlackFox\User($random_user_id);
